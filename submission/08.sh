...
if [ -z "$VOUT_INDEX" ]; then
  echo "ERROR: no suitable vout" >&2; exit 1
fi

UTXO_SATS=$(bitcoin-cli -regtest decoderawtransaction "$BASE_TX" | jq -r ".vout[$VOUT_INDEX].value" | awk '{printf "%.0f\n",$1*1e8}')
PAY_SATS=$((UTXO_SATS - FEE_SATS))
if [ "$PAY_SATS" -le 0 ]; then
  echo "ERROR: insufficient funds after fee" >&2; exit 1
fi

toBTC(){ awk "BEGIN{printf \"%.8f\", $1/1e8}"; }
PAY_BTC=$(toBTC $PAY_SATS)

INPUTS="[{\"txid\":\"$TXID\",\"vout\":$VOUT_INDEX}]"
OUTPUTS="{\"$TARGET\":$PAY_BTC}"
