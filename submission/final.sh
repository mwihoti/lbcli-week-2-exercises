...
# WRITE YOUR SOLUTION BELOW:
FIRST_OUTPUT_VALUE=$(bitcoin-cli -regtest decoderawtransaction "$BASE_TX" | jq -r '.vout[0].value * 1e8 | floor')
...
UTXO_VALUE=$(bitcoin-cli -regtest decoderawtransaction "$BASE_TX" | jq -r '.vout['"$UTXO_VOUT_INDEX"'].value * 1e8 | floor')
...
PAYMENT_BTC=$(awk "BEGIN{printf \"%.8f\", $PAYMENT_AMOUNT/1e8}")
CHANGE_BTC=$(awk "BEGIN{printf \"%.8f\", $CHANGE_AMOUNT/1e8}")
...
VERIFY_PAYMENT_RAW=$(echo "$DECODED_TX" | jq -r --arg addr "$PAYMENT_ADDRESS" '.vout[] | select(.scriptPubKey.address == $addr) | .value' | head -n 1)
VERIFY_PAYMENT=$(awk "BEGIN{printf \"%.8f\", $VERIFY_PAYMENT_RAW}")
check_cmd "Payment verification" "VERIFY_PAYMENT" "$VERIFY_PAYMENT"

VERIFY_CHANGE_RAW=$(echo "$DECODED_TX" | jq -r --arg addr "$CHANGE_ADDRESS" '.vout[] | select(.scriptPubKey.address == $addr) | .value' | head -n 1)
VERIFY_CHANGE=$(awk "BEGIN{printf \"%.8f\", $VERIFY_CHANGE_RAW}")
...
# WRITE YOUR SOLUTION BELOW:
CHANGE_OUTPUT_INDEX=$(echo "$DECODED_TX" | jq -r --arg addr "$CHANGE_ADDRESS" '.vout | to_entries[] | select(.value.scriptPubKey.address == $addr) | .key' | head -n 1)
...
# Convert to BTC
CHILD_SEND_BTC=$(awk "BEGIN{printf \"%.8f\", $CHILD_SEND_AMOUNT/1e8}")
...
SECONDARY_OUTPUT_VALUE=$(bitcoin-cli -regtest decoderawtransaction "$SECONDARY_TX" | jq -r '.vout[0].value * 1e8 | floor')
...
# Convert to BTC
TIMELOCK_BTC=$(awk "BEGIN{printf \"%.8f\", $TIMELOCK_AMOUNT/1e8}")
